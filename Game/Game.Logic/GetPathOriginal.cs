...
namespace Game.Logic
{
    public class GetPathOriginal : IPathStrategy
    {
        ...
        {
            int[,] visited = new int[field.Height, field.Width];
            int step = 0;
            ...
            while (visitedPositions.Count != 0)
            {
                ...
                visited[dequeue.Item1.Row, dequeue.Item1.Column] = dequeue.Item2;
                var neighbors = field.GetNeighbors(dequeue.Item1);

                foreach (var neighbor in neighbors)
                {
                    if (visited[neighbor.Row, neighbor.Column] == 0 && field.GetBallColorAt(neighbor.Row, neighbor.Column) == BallColor.Empty)
                    {
        ...
        private List<Position> GetPathBack(Position source, Position destination, int[,] visited)
        {
            ...
            {
                var neighbors = field.GetNeighbors(destination);
                foreach (var neighbor in neighbors)
                {
                    if (neighbor == source)
                    {
